...
if [ -z "$3" ]; then
  echo -e "WARNING!!\nYou need to pass the WEBHOOK_URL environment variable as the third argument to this script." && exit
fi
...
(curl --fail --progress-bar -A "TravisCI-Webhook" -H Content-Type:application/json -H X-Author:k3rn31p4nic#8383 -d "${WEBHOOK_DATA//	/ }" "$3" \
